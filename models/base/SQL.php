...
class SQL implements IDatabase
{
    ...
    /**
     * Retourne le nombre d'enregistrements de la table (pour la table $tableName)
     * @return int
     */
    public function getNbRecords(): int
    {
        $query = $this->pdo->prepare("SELECT COUNT(*) FROM $this->tableName");
        $query->execute();
        $result = $query->fetch();
    ...
    /**
     * Permet la création d'une entrée dans une table
     */
    public function insertOne($data = array()) {
        ...
        $str="INSERT INTO {$this->tableName} (".implode(',',$columns).") VALUES ('" . implode("', '", $values) . "' )";
